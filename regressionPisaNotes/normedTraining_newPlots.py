...
new_listofVars=[
    'Jet_rawPt','Jet_mt','Jet_eta','Jet_mass','Jet_ptd',
    ...
    "Jet_mcPt", "Jet_pt_reg", "Jet_corr", "Jet_pt"
    ]

data_len=len(new_listofVars)-4
all_len=len(new_listofVars)-3
...
print regTree.shape
print regTree[0]
print regTree[0,0], regTree[0,-1]
print "smearing", regTree[0,0]*regTree[0,-2]/regTree[0,-1]
regTree[:,3]=regTree[:,3]*regTree[:,0]*regTree[:,-2]/regTree[:,-1]
regTree[:,1]=regTree[:,1]*regTree[:,0]*regTree[:,-2]/regTree[:,-1]
regTree[:,0]=regTree[:,0]*regTree[:,-2]
print regTree[0]
...
class plotHistory(keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}): 
            ...
            prediction=prediction*v_data[:,0:1]
            ...
            plt.hist(v_data[inds][:,data_len:all_len]/prediction[inds][:,0:1], bins=200, range=[-0.5,2.5], alpha=1, color='red', edgecolor='red', label="mc/reg no sm.", histtype='stepfilled')
            ...
            plt.hist(v_data[inds][:,data_len:all_len]/v_data[inds][:,0:1], bins=200, range=[-0.5,2.5], alpha=0.5, color='blue', edgecolor='blue', label="mc/pt no sm.", histtype='step')
            plt.legend(loc=1)
            plt.grid(True, which='both')
            ...
            plt.hist(v_data[inds2][:,data_len:all_len]/prediction[inds2][:,0:1], bins=200, range=[-0.5,2.5], alpha=1, color='red', edgecolor='red', label="mc/reg no sm.", histtype='stepfilled')
            ...
            plt.hist(v_data[inds2][:,data_len:all_len]/v_data[inds2][:,0:1], bins=200, range=[-0.5,2.5], alpha=0.5, color='blue', edgecolor='blue', label="mc/pt no sm.", histtype='step')
            plt.legend(loc=1)
            plt.grid(True, which='both')
            ...
            plt.hist(v_data[inds3][:,data_len:all_len]/prediction[inds3][:,0:1], bins=200, range=[-0.5,2.5], alpha=1, color='red', edgecolor='red', label="mc/reg no sm.", histtype='stepfilled')
            ...
            plt.hist(v_data[inds3][:,data_len:all_len]/v_data[inds3][:,0:1], bins=200, range=[-0.5,2.5], alpha=0.5, color='blue', edgecolor='blue', label="mc/pt no sm.", histtype='step')
            plt.legend(loc=1)
            plt.grid(True, which='both')
...
##x=  Dense(20, activation='relu',init='lecun_uniform')(x)
###x = Dropout(dropoutRate)(x)
##predictions = Dense(1,init='lecun_uniform')(x)
...
history=model.fit(scaled_t_data , t_data[:,data_len:all_len]/t_data[:,0:1], nb_epoch=200, verbose=1,batch_size=5120, 
                  callbacks=[wHistory_, pltH_, reduce_lr], validation_data=(scaled_v_data , v_data[:,data_len:all_len]/v_data[:,0:1]))
