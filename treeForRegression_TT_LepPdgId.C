...
void treeForRegression_TT_LepPdgId()
{
...
//	 V_v6-v1/171120_094251/0000/
        std::string inputfilename="/uscms_data/d3/cvernier/optimizationRegression/RegressionHbb/newTT.root";///eos/uscms/store/user/lpchbb/HeppyNtuples/V25/ZH_HToBB_ZToLL.root";//allgrav.root";//ttbar-g-25.root";
///eos/uscms/store/group/lpchbb/HeppyNtuples/V21/ZH_HToBB_ZToLL_M125_13TeV_powheg_pythia8.root";
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		for(int i =0 ; i<nJet; i++){
			...
			Jet_leptonPdgId_=-1;
			if(Jet_leptonPdgId[i]==13||Jet_leptonPdgId[i]==-13)Jet_leptonPdgId_ = 1;
			if(Jet_leptonPdgId[i]==11||Jet_leptonPdgId[i]==-11)Jet_leptonPdgId_ = 0;
			...
			if (tomatch==0) {std::cout<< "NO match "<< (Jet_leptonDeltaEta_+Jet_eta_)<< " "<< (Jet_leptonDeltaPhi_+Jet_phi_)<< "  "<< Jet_leptonPt_<< " "<< jentry<<std::endl; for (int m=0; m<naLeptons; m++){if (aLeptons_pt[m]== Jet_leptonPt_)  std::cout<<aLeptons_pt[m]<<" "<<aLeptons_eta[m]<<" "<<aLeptons_phi[m]<<std::endl;}}
